...
import { DEXSwapPool } from "./DEXSwapPool.sol";
...
contract DEXSwapPoolDeployer is IDEXSwapPoolDeployer {
    struct Parameters {
    ...
    function deploy(
        ...
        address lpToken = address(new DEXSwapLiquidityProviderERC20(name, symbol, address(this)));
        ...
        address pool = address(new DEXSwapPool{salt: keccak256(abi.encode(token0, token1, fee))}());
        
        // Transfer ownership of the LP token contract to the newly deployed pool.
        DEXSwapLiquidityProviderERC20(lpToken).transferOwnership(pool);
